...
void init_lvgl_add_medicine_scr(medicine_t *medicine)
{
    ...
    if (medicine == NULL)
    {
        ...
        medicine->treatment_end_date.tm_sec = 0;
        medicine->treatment_end_date.tm_min = 0;
        medicine->treatment_end_date.tm_hour = 0;

        medicine->treatment_end_date.tm_mday += 1 % 31;
        mktime(&medicine->treatment_end_date); // Normalize the date
        strcpy(medicine->special_requirements, "Special requirements: none.");
    ...
    lv_obj_add_event_cb(name_btn, name_btn_event_handler, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(frequency_btn, frequency_btn_event_handler, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(start_date_btn, start_date_btn_event_handler, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(end_date_btn, end_date_btn_event_handler, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_add_event_cb(special_req_btn, special_req_btn_event_handler, LV_EVENT_CLICKED, NULL);
...
static void show_calendar(bool is_start_date)
{
    ...
    struct tm *current_date = is_start_date ? &local_medicine->treatment_start_date : &local_medicine->treatment_end_date;
...
static void save_btn_event_handler(lv_event_t *e)
{
    ...
    if (is_new_medicine)
    {
        set_medicine_times_from_frequency(local_medicine);
